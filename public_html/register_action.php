...
function redirectToRegister(string $errorCode, array $old = []): void {
    ...
    header('Location: /?page=register&error=' . urlencode($errorCode));
    exit;
}
...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    redirectToRegister('invalidEmail', $old);
}

if (!preg_match('/^[A-Za-z0-9]{1,15}$/', $name)) {
    redirectToRegister('invalidName', $old);
}

if (mb_strlen($pass, 'UTF-8') < 6) {
    redirectToRegister('weakPass', $old);
}
...
if (!$dobDate || $dobDate->format('Y-m-d') !== $dob) {
    redirectToRegister('invalidDob', $old);
}
...
if ($age < 18) {
    redirectToRegister('underAge', $old);
}
